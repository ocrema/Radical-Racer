...
class HUD(Entity):
    def __init__(self, **kwargs):
        ...
        self.speedometer = Sprite(model='quad', texture='speedometer.png', scale=.06, parent=self, position=(-.7,-.35))
        self.speed_text = Text(position=(-.78, -.42), scale=1.5, parent=self)
        self.speedometer_pointer = Sprite(model='quad', texture='speedometer_pointer.png', scale=.03, parent=self, position=(-.7, -.35))
        self.track = Sprite(model='track_road', parent=self, position=(.7, -.3, -1), rotation_x=0, rotation_y=90, rotation_z=90, scale=.018, color=color.gray)
        ...
        self.open_controls_text = Text(position=(-.85, .48), scale=1, parent=self, text="Hold 'tab' for controls")
        self.restart_text = Text(position=(-.85, .45), scale=1.5, parent=self, text="'R' to start race")
        self.lap_text = Text(position=(-.85, .4), scale=1.5, parent=self)
        self.checkpoint_text = Text(position=(-.85, .35), scale=1.5, parent=self)
        self.time_text_background = Sprite(model='quad', color=color.dark_gray, parent=self, scale=(.4, .1), unlit=True,
                                           position=(.5, .42), visible=False)
        self.time_text = Text(position=(.5, .42), origin=(0,0), scale=3, parent=self)
        self.win_flag_left = Sprite(model='quad', texture='checkerboard.png', parent=self, scale=(4*.25, 5*.25), unlit=True,
                                           position=(.25, .42), visible=False)
        self.win_flag_right = Sprite(model='quad', texture='checkerboard.png', parent=self, scale=(4*.25, 5*.25), unlit=True,
                                    position=(.75, .42), visible=False)
